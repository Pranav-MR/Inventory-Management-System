...
### Notifications
`notifications/channels/NotificationChannel.ts` defines the interface; `EmailChannel.ts` is the only implementation (falls back to `nodemailer`'s `jsonTransport` + console logging when `SMTP_HOST` is unset, so it never hard-fails in local dev). `notifications/registry.ts` maps `NotificationChannelType` → implementation — adding SMS/WhatsApp (Twilio) later means writing one more channel class and one registry line, no changes to `notification.service.ts`. Dispatch is deduped via `NotificationLog.dedupeKey` (`itemId:eventType:channel:day`), which is what makes the daily cron job idempotent across reruns on the same day — tests for new notification triggers should assert on this idempotency, not just on the notification firing once.

### Daily job
`jobs/dailyProjectionJob.ts` exports `runDailyProjectionJob()` as a plain async function (not tied to `node-cron`) specifically so it can be invoked directly in tests and later moved to a real queue/worker without touching its internals. `jobs/scheduler.ts` is the only place `node-cron` is imported.
